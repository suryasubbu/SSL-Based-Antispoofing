...
def read_pickle_files(base_directory, feature_type, speaker_no, train_type):
    ...
    speaker_path = os.path.join(base_directory, 'CODEC2', speaker_no, train_type)
...
def visualize_umap(data, labels, feature_type, output_directory, model_no, results_df, speaker_no):
    ...
    if not os.path.exists(png_output_path):
        ...
        # Add X marks for the closest points for each label
        ...
        #     fig.add_scatter(
        #         x=[closest_point['UMAP1']],
        #         y=[closest_point['UMAP2']],
        #         mode='markers+text',
...
"p282",]
    ...
    for speaker_no in speaker_nos:
        train_type = "train"
        output_directory = f"/data/Deep_Fake_Data/umap_plots_closest_CODEC2/{speaker_no}"  # Directory to save plots
    ...
    results_output_path = os.path.join(base_directory, "CODEC2_xv_umap_distances_closest_each_summary.csv")
