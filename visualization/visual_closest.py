...
def calculate_distances_from_fixed_label(visualization_df, fixed_label):
    """
    Calculate distances from a fixed label cluster to all other clusters using closest points and their average.
    ...
    # Calculate distances from the closest point in the fixed label to all other clusters
    distances = {}
    for other_label in visualization_df['label'].unique():
        ...
        min_distance = min(
            euclidean(fixed_point, other_point)
            for fixed_point in fixed_label_points
            for other_point in other_label_points
        )
        distances[f"Distance_{fixed_label}_{other_label}"] = min_distance
...
"p282",]
    ...
    results_output_path = os.path.join(base_directory, "umap_distances_closest_summary.csv")
